...
sealed class Operation(val type: Type) {
    ...
    sealed class Local(type: Type) : Operation(type) {
         ...
         */
        data class Invalidate(val useRequestParameters: Boolean = true) : Local(INVALIDATE) {
            override fun toString() = SERIALISER.serialise(
                    type,
         ...
         */
        data class Clear(val useRequestParameters: Boolean = true,
                         val clearStaleEntriesOnly: Boolean = false) : Local(CLEAR) {
            override fun toString() = SERIALISER.serialise(
                    type,
                    useRequestParameters,
