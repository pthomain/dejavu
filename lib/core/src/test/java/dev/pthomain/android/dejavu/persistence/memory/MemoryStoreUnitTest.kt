...
internal class MemoryStoreUnitTest : BaseKeyValueStoreUnitTest<dev.pthomain.android.dejavu.persistence.memory.MemoryStore>() {
    ...
    override fun setUpTarget(): dev.pthomain.android.dejavu.persistence.memory.MemoryStore {
        mockLruCache = mock()
        mockLruCacheFactory = mock()

        whenever(mockLruCacheFactory.invoke(eq(20))).thenReturn(mockLruCache)

        return dev.pthomain.android.dejavu.persistence.memory.MemoryStore.Factory(mockLruCacheFactory).create(20)
    }
